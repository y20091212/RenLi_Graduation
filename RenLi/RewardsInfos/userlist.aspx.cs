...
public partial class RewardsInfos_userlist : System.Web.UI.Page
{
    ...
    {
        StringBuilder strSql = new StringBuilder();
        strSql.Append("select RewardsInfos.ID,JiannChengId,	case JiannChengId	WHEN '1' THEN '奖励' WHEN '2' THEN '项目提成'	WHEN '0' THEN '惩罚'	END AS JiannCheng	,Money,UserId,UserInfos.UserName,LuRuUserId,luruusers.UserName as LuRuUserName,Remark,RewardsInfos.SubTime ");
        ...
        strSql.Append(" where UserId = "+Session["userid"].ToString());
        if (strWhere.Trim() != "")
        {
